function nwb = add_processed_ophys(nwb, metadata, image_masks, ...
	...
	% changelog
		...
		% 2020.05.11 [21:06:41] - Added more checks and conversion for single datatypes.
	...
	op_input_args = nwbpkg.get_input_args(metadata.Ophys.ImagingPlanes{1}, 'optical_channels');
	...
	for i = 1:numOfPlanes
		...
		ip_input_args = nwbpkg.get_input_args(metadata.Ophys, 'ImagingPlanes');
	...
	for i = 1:numOfPlanes
		ps_input_args = nwbpkg.get_input_args(metadata.Ophys.ImageSegmentation.plane_segmentations{i}, '');
	...
	for ser = 1:NumOfSeries
		roi_response_series_varargin{ser} = nwbpkg.get_input_args(metadata.Ophys.DFOverF.roi_response_series{ser}, '');
	end
